...
/**
 * Imprint Stop Hook — smart memory extraction, stored in Supermemory Local.
 ...
 *
 * Every extracted fact is saved to the Supermemory server (Local at
 * localhost:6767 by default — nothing leaves the machine). Supermemory handles
 * embeddings, de-duplication, and versioning server-side. Set
 * IMPRINT_INGEST_TRANSCRIPT=1 to ALSO hand the raw dialogue to Supermemory's
 * own extraction pipeline (POST /v3/documents) and let the memory engine do
 * the remembering itself.
 */
...
const LAST_ACTIVITY_FILE = join(tmpdir(), `imprint-last-activity-${CONTAINER_TAG}.json`);
...
// ── Save a fact ───────────────────────────────────────────
// Single save (used for the AFK summary). Supermemory de-duplicates and
// versions server-side, so best-effort re-saves are safe.
async function saveFact({ content, topic }) {
  try {
    await saveMemory({ content, topic: topic || "general", pinned: false, source: "stop-hook" });
  } catch (e) {
    process.stderr.write(`[Imprint hook] save failed: ${e.message}\n`);
  }
}

// Save several extracted facts in ONE batched Supermemory call.
...
async function main() {
  try {
    ...
    // Optionally hand the raw dialogue to Supermemory's own extraction
    // pipeline — it chunks, embeds, and derives memories from the document.
    if (INGEST_TRANSCRIPT) {
      try {
        await ingestDocument(recent, { source: "stop-hook-transcript" });
      } catch (e) {
        process.stderr.write(`[Imprint hook] transcript ingest failed: ${e.message}\n`);
      }
    }
    ...
    // One batched Supermemory save for all facts; the server de-duplicates.
